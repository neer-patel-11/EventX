...
def update_password(user_id: int, new_password: str, db: Session = Depends(get_db)):
    updated = user_service.update_user_password(db, user_id, new_password)
    if not updated:
        raise HTTPException(status_code=404, detail="User not found")
...
def update_admin_status(user_id: int, is_admin: bool, db: Session = Depends(get_db)):
    updated = user_service.update_user_admin_status(db, user_id, is_admin)
    if not updated:
        raise HTTPException(status_code=404, detail="User not found")
...
def set_balance(user_id: int, new_balance: int, db: Session = Depends(get_db)):
    updated = user_service.update_user_balance(db, user_id, new_balance)
    if not updated:
        raise HTTPException(status_code=404, detail="User not found")
    return updated
...
def add_balance(user_id: int, amount: int, db: Session = Depends(get_db)):
    updated = user_service.add_to_user_balance(db, user_id, amount)
    if not updated:
        raise HTTPException(status_code=404, detail="User not found")
    return updated
...
def deduct_balance(user_id: int, amount: int, db: Session = Depends(get_db)):
    updated = user_service.deduct_from_user_balance(db, user_id, amount)
    if updated is False:
...
def get_users_with_high_balance(min_balance: int, db: Session = Depends(get_db)):
    return user_service.get_users_with_balance_above(db, min_balance)
...
def get_users_with_low_balance(max_balance: int, db: Session = Depends(get_db)):
    return user_service.get_users_with_balance_below(db, max_balance)


@router.get("/count/total")
def total_users(db: Session = Depends(get_db)):
    return {"total_users": user_service.get_user_count(db)}

@router.get("/count/admin")
def total_admins(db: Session = Depends(get_db)):
    return {"total_admins": user_service.get_admin_count(db)}
